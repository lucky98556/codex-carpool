...
const loadingTargets={full:['#stats','.key-panel>.table-wrap','.key-analysis','#official-account-pool','.logs'],key:['.key-analysis','#decision-log-view'],analysis:['.key-analysis'],decision:['#decision-log-view'],operation:['#operation-log-view'],accounts:['#official-account-pool'],catalog:['.key-panel>.table-wrap','.key-analysis']};
...
function policyFor(id){return state.policies.find(item=>item.id===id)}function keyFor(id){return state.summary.keys.find(item=>item.id===id)}function selectKey(id){state.selected=text(id);try{if(state.selected)localStorage.setItem(selectedKeyStorage,state.selected);else localStorage.removeItem(selectedKeyStorage)}catch{}}function suffix(key){return key?.fingerprint_prefix?'••••'+String(key.fingerprint_prefix).slice(-4):'••••'}function allocationX(window,name){const explicit=Number(window?.[name+'_x']);if(Number.isFinite(explicit))return Math.max(0,explicit);return Math.max(0,(Number(window?.[name])||0)/1000000)}function percent(window){const capacity=allocationX(window,'capacity');return capacity?Math.min(100,Math.max(0,allocationX(window,'used')/capacity*100)):0}function meter(window,green=false){return '<div class="meter '+(green?'green':'')+'"><i style="width:'+percent(window)+'%"></i></div>'}function formatTime(value){return new Date(value).toLocaleString(state.locale==='en'?'en-US':'zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit',hour12:false})}
...
function renderKeys(){
	const allKeys=state.summary.keys||[],filter=text($('search').value).toLowerCase(),keys=allKeys.filter(key=>(key.name+' '+key.fingerprint_prefix).toLowerCase().includes(filter));
...
const openLogDetail=(log,row)=>{
	if(!log)return;
	const cells=row?.children||[],key=keyFor(log.key_id)||keyFor(state.selected);
	...
	set('log-detail-key-id',log.key_id);
...
(()=>{const validTabs=new Set(['decision','operation']),copy=tab=>tab==='operation'?uiText('插件生命周期、配置、官方额度同步与异常记录','Plugin lifecycle, configuration, official quota sync, and exception records'):uiText('当前 Key 的使用、策略决策与 Token 结算记录','Usage, policy decisions, and Token settlements for the selected Key');const activate=tab=>{if(!validTabs.has(tab))return;document.querySelectorAll('[data-log-tab]').forEach(button=>{const selected=button.dataset.logTab===tab;button.classList.toggle('active',selected);button.setAttribute('aria-selected',String(selected))});const decisionView=document.getElementById('decision-log-view'),operationView=document.getElementById('operation-log-view'),decisionTools=document.getElementById('decision-log-tools'),operationTools=document.getElementById('operation-log-tools'),description=document.getElementById('log-tab-description');if(decisionView)decisionView.hidden=tab!=='decision';if(operationView)operationView.hidden=tab!=='operation';if(decisionTools)decisionTools.hidden=tab!=='decision';if(operationTools)operationTools.hidden=tab!=='operation';if(description)description.textContent=copy(tab);document.dispatchEvent(new CustomEvent('codex-carpool:log-tab-changed',{detail:{tab}}))};document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target.closest('[data-log-tab]'):null;if(!target)return;event.preventDefault();activate(target.dataset.logTab)},true);window.__ccActivateLogTab=activate;activate('decision')})();
